...
public class PaxController extends DeviceController {
	...
	public IDeviceCommInterface configureConnector() throws ConfigurationException {
		...
		case TCP_IP:
			return new PaxTcpInterface(settings);
		case HTTP:
			return new PaxHttpInterface(settings);
		case SERIAL:
		case SSL_TCP:
		default:
	...
	public ITerminalResponse processTransaction(TerminalAuthBuilder builder) throws ApiException {
			...
			if (builder.getPaymentMethod() instanceof CreditCardData) {
				...
				if (StringUtils.isNullOrEmpty(card.getToken())) {
					...
					if (builder.getTransactionType() != TransactionType.Verify
							&& builder.getTransactionType() != TransactionType.Refund)
						account.setCvvCode(card.getCvn());
				} else
					extData.set(PaxExtData.TOKEN, card.getToken());
		...
		case Credit:
			return (ITerminalResponse) doCredit(transType, amounts, account, trace, avs, cashier, commercial, ecom,
					extData);
		case Debit:
			return (ITerminalResponse) doDebit(transType, amounts, account, trace, cashier, extData);
		case Gift:
			PaxMsgId messageId = builder.getCurrency() == CurrencyType.Currency ? PaxMsgId.T06_DO_GIFT
					: PaxMsgId.T08_DO_LOYALTY;
			return (ITerminalResponse) doGift(messageId, transType, amounts, account, trace, cashier, extData);
		case EBT:
			if (builder.getCurrency() != null)
				account.setEbtType(builder.getCurrency().getValue().substring(0, 1));
			return (ITerminalResponse) doEbt(transType, amounts, account, trace, cashier);
		default:
	...
	public ITerminalResponse manageTransaction(TerminalManageBuilder builder) throws ApiException {
		...
		switch (builder.getPaymentMethodType()) {
		case Credit:
			return (ITerminalResponse) doCredit(transType, amounts, account, trace, new AvsRequest(),
					new CashierSubGroup(), new CommercialRequest(), new EcomSubGroup(), extData);
		case Gift:
			PaxMsgId messageId = builder.getCurrency() == CurrencyType.Currency ? PaxMsgId.T06_DO_GIFT
					: PaxMsgId.T08_DO_LOYALTY;
			return (ITerminalResponse) doGift(messageId, transType, amounts, account, trace, new CashierSubGroup(),
					extData);
		case EBT:
			return (ITerminalResponse) doEbt(transType, amounts, account, trace, new CashierSubGroup());
		default:
			throw new UnsupportedTransactionException();
		}
	}
	...
	private PaxTxnType mapTransactionType(TransactionType type, boolean requestToken)
	...
	private byte[] doTransaction(PaxMsgId messageId, PaxTxnType transactionType, IRequestSubGroup... subGroups)
			throws ApiException {
	...
	public CreditResponse doCredit(PaxTxnType transactionType, AmountRequest amounts, AccountRequest accounts,
			TraceRequest trace, AvsRequest avs, CashierSubGroup cashier, CommercialRequest commercial,
			EcomSubGroup ecom, ExtDataSubGroup extData) throws ApiException {
		byte[] response = doTransaction(PaxMsgId.T00_DO_CREDIT, transactionType, amounts, accounts, trace, avs, cashier,
				commercial, ecom, extData);
		return new CreditResponse(response);
	}

	public GiftResponse doGift(PaxMsgId messageId, PaxTxnType transactionType, AmountRequest amounts,
			AccountRequest accounts, TraceRequest trace, CashierSubGroup cashier, ExtDataSubGroup extData)
			throws ApiException {
	...
	public DebitResponse doDebit(PaxTxnType transactionType, AmountRequest amounts, AccountRequest accounts,
			TraceRequest trace, CashierSubGroup cashier, ExtDataSubGroup extData) throws ApiException {
		byte[] response = doTransaction(PaxMsgId.T02_DO_DEBIT, transactionType, amounts, accounts, trace, cashier,
				extData);
		return new DebitResponse(response);
	}

	public EbtResponse doEbt(PaxTxnType transactionType, AmountRequest amounts, AccountRequest accounts,
			TraceRequest trace, CashierSubGroup cashier) throws ApiException {
		byte[] response = doTransaction(PaxMsgId.T04_DO_EBT, transactionType, amounts, accounts, trace, cashier,
				new ExtDataSubGroup());
