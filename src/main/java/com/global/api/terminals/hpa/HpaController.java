...
public class HpaController extends DeviceController implements IDisposable {
	...
	public IDeviceCommInterface configureConnector() throws ConfigurationException {
		...
		case TCP_IP:
			return new HpaTcpInterface(settings);
		default:
	...
	private <T extends SipBaseResponse> T sendMessage(Class<T> clazz, String message, String... messageIds)
			throws ApiException {
		return sendMessage(clazz, message, false, true, messageIds);
	}

	private <T extends SipBaseResponse> T sendMessage(Class<T> clazz, String message, boolean keepAlive,
			boolean awaitResponse, String... messageIds) throws ApiException {
		...
		byte[] response = _connector.send(deviceMessage);
		if (awaitResponse) {
			try {
	...
	<T extends SipBaseResponse> T sendAdminMessage(Class<T> clazz, HpaAdminBuilder builder) throws ApiException {
		...
		return sendMessage(clazz, builder.buildMessage(), builder.isKeepAlive(), builder.isAwaitResponse(),
				builder.getMessageIds());
	}
	...
	public ITerminalResponse processTransaction(TerminalAuthBuilder builder) throws ApiException {
		...
		return (ITerminalResponse) sendMessage(SipDeviceResponse.class, et.toString(request), transactionType);
	}
